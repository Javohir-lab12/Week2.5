...
class LengthValidator(Validator):
    def __init__(self, min_val, max_val):
        super().__init__(f"Length({min_val}-{max_val})")
        self.max_val = max_val
        self.min_val = min_val
    def validate(self, value):
        result = self.min_val <= len(value) <= self.max_val
        return result
...
class StartsWithUpperValidator():
    ...
    def validate(self, value):
        return bool(value) and value[0] == value[0].upper()
    def check(self, value):
        result = self.validate(value)
        status = "PASS" if result else "FAIL"
        print(f"[{status}] {self.name}: {value}")
...
class ValidationReport:
    ...
    def summary(self):
        ...
        passed = len([entry for entry in self.entries if entry[2]])
        failed = total - passed
